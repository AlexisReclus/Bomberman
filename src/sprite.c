...
#define EXPLOSION		"sprite/explosion2.png"
...
static void bomb_load() {
	bomb[0]=load_image(BOMB_TTL4);
	bomb[1]=load_image(BOMB_TTL3);
	bomb[2]=load_image(BOMB_TTL2);
	bomb[3]=load_image(BOMB_TTL1);
}
...
SDL_Surface * sprite_get_bomb(int number){
	assert(0<= number && number <5);
	return bomb[number];
}
